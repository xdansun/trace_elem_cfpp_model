function boot_cq_TE = boot_coal_blend_conc_verify_dg(coal_gen_boiler_apcd, cq_hg_2015, cq_se_2015, ...
    cq_as_2015, cq_cl_2015, trials)
% Uses Daniel Gingerich's approach to estimate trace element concentrations
% in coal 
...
for i = 1:size(plant_list,1) % for each coal plant 
    if sum(plant_list_cq_dist == plant_list(i)) > 0 
        ...
        coal_conc = zeros(size(plant_purch,1),1);
        coal_blend_conc = zeros(1,1); 
        for j = 4:(3+size(plant_purch,1)) % coal distribution starts on index 4, 
            ...
            % the correct to add because for 5 counties, 4-8 is correct indexing
             coal_dist = median(plant_data{1,j}); 
