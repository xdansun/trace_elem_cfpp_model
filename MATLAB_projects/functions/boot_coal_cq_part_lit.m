function [plt_blr_TE_emis, plt_TE_emis, plt_blr_TE_input] = boot_coal_cq_part_lit(...
    coal_gen_boiler_apcd, boot_cq_TE, boot_part_TE, ann_coal_gen, poll)
%% description
% estimate generation normalized mass flow rates of trace elements  at the
% boiler level and plant level
%
% inputs
...
% ann_coal_gen (int) - total coal generation in 2015 
% poll (string) - pollutant or trace element we perform calculation for 
% 
% outputs 
% plt_blr_TE_emis (table) - Table of mass flow rates and generation
% normalized mass flow rates at the boiler level to solid, liquid, and gas
% with generation at the boiler
% plt_TE_emis (table) - Table of mass flow rates and generation
% normalized mass flow rates at the plant level to solid, liquid, and gas
% with generation at the boiler
% plt_blr_TE_input (table) - Table of the mass flow rate of trace elements 
% entering the boiler in kg
...
boot_remov_TE_tbl = cell2table(boot_part_TE); 
...
for i = 1:size(plt_blr_TE_emis,1)
    ...
    annual_loadings(:,4) = annual_loadings(:,4)+ prctile(total_emis, [25, 50, 75])'/1e9;
end 
